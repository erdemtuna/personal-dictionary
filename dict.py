...
client = MongoClient('<Connection String Obtained from your MongoDB account')
db = client.get_database('<Your Database Name')
#collection name inside your database
trans = db.translations

#print(trans.count_documents({}))

dict_keys = ('word', 'means', 'use')
states = ('1', 'word', '2', 'quit')
...
while not (QUIT):
    #determine the state
    user_in = input('Word(1) or Quit(2): ')
    ...
    if(user_low == states[0] or user_low == states[1]):
        ...
        while(more):
            ...
            for  key in dict_keys:
                if(key == dict_keys[0]):
                    ...
                    command_in = input('Want to add more words? (Y)es or (N)o?')
        ...
        json_array = json.loads(json.dumps(temp_array, ensure_ascii = False).encode('utf8'))
        #insert to database
        print(json_array)
